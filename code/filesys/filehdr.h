...
//#define numIdxBlock ((SectorSize - 2 * sizeof(int)) / sizeof(int))

#define numIdxEntry ((SectorSize - sizeof(int)) / sizeof(int))
...
private:
	...
	int numSectors; // Number of data sectors in the file
	int startSector;
	int useless[NumDirect]; // Disk sector numbers for each data
...
public:
	...
	~IndexBlock();
	int nextSector;
	void Allocate(PersistentBitmap *freeMap, int numEntry);
...
private:
	int dataSectors[numIdxEntry]; // Disk sector numbers for each data
																// block in the file
};
