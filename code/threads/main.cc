...
static void Copy(char *from, char *to)
{
    ...
    if (!kernel->fileSystem->Create(to, fileLength, 1)) //陳柏均改的
    {                                                   // Create Nachos file
...
//----------------------------------------------------------------------
static void CreateDirectory(char *name) //陳柏均寫的
{

    kernel->fileSystem->Create(name, DirectoryFileSize, 0);

    OpenFile *dirFile = kernel->fileSystem->Open(name);
